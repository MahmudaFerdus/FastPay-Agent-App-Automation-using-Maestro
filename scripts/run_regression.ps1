...
Write-Host "  FastPay Agent - Automation Framework  " -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# ─── PREPARE PATHS & VARIABLES ───
$suitePath = "$rootDir\suites\regression.yaml"
$envPath = "$rootDir\.env"
...
# This ensures Windows executes the command exactly as if a human typed it
$tempBatFile = "$reportDir\run_maestro.bat"
...
$flows = @(
    ...
    @{ Name = "Sell Balance (< 250 IQD)";  File = "sell_balance_less_than_250.yaml"; Type="Negative" },
    @{ Name = "Transfer Money (Success)";  File = "transfer_success.yaml";           Type="Positive" },
    @{ Name = "Transfer Money (Invalid)";  File = "transfer_invalid_number.yaml";    Type="Negative" }
)
...
$bannerText = if ($failedCount -gt 0) {"&#10060; SOME TESTS FAILED"} elseif ($skippedCount -gt 0) {"&#9888; SOME TESTS SKIPPED"} else {"&#9989; ALL TESTS PASSED"}

$html = @"
<!DOCTYPE html><html><head><title>FastPay Automation Report</title>
<style>
...
th{text-align:left;padding:18px 15px;background:#2d3748;color:white;font-weight:600;text-transform:uppercase;font-size:13px;letter-spacing:1px}
...
<div class='container'>
    <div class='header'>
        <h1>FastPay Agent Automation Report</h1>
...
$htmlPath = "$reportDir\FastPay_Report_$timestamp.html"
$html | Out-File -FilePath $htmlPath -Encoding UTF8
Write-Host "`nBeautiful HTML Report Generated: $htmlPath" -ForegroundColor Green
